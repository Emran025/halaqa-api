...
| وثائق المعلم | `teacher_documents` | `TeacherDocumentInput`, `TeacherDocumentUploadInput`, `TeacherDocument` | `POST/PATCH/DELETE /me/teacher-profile/documents` | `ManageTeacherDocumentService`, `TeacherDocumentResource` |
...
| طلب التسجيل | `registration_requests`, `registration_request_profiles` | `CreateRegistrationInput`, `RegistrationRequest`, `RegistrationResponse` | `POST /registrations`, صندوق الطلبات، القبول والرفض | `SubmitRegistrationService`, `AcceptRegistrationService` |
...
|---|---|---|---|---|
| ملف التوافر الحالي | `student_availability_profiles`, `student_availability_slots` | `AttendancePreferences` | `GET/PATCH /me/student-availability` | `UpdateStudentAvailabilityService` |
| خطة الطالب | `follow_up_plans`, `follow_up_plan_details` | `FollowUpPlanInput`, `FollowUpPlan`, `PlanDetail` | `/students/{studentId}/follow-up-plan` | `CreateFollowUpPlanService`, `UpdateFollowUpPlanService` |
| عنصر المتابعة | `follow_up_items` | `FollowUpItem`, `FollowUpItemResponse` | قائمة اليوم، إكمال، تجاوز، إعادة جدولة | `FollowUpQueueQuery`, `CompleteFollowUpItemService`, `RescheduleFollowUpService` |
...
|---|---|---|---|
| إصدار المصحف | `quran_editions` | `QuranEdition` إن لزم | إدارة مرجع داخلي/تحميل النسخة |
...
| الخطأ | `mistakes` | `CreateMistakeInput`, `UpdateMistakeInput`, `Mistake` | إنشاء وتعديل وحذف منطقي وعرض فوري، مع `tracking_detail_id` و`ayah_id` و`word_index` و`mistake_type_id`. |
...
| مهمة الجلسة | `session_tasks` | `SessionTask`, `CreateTaskInput` | إنشاء وتحديث وإكمال المهمة | `SessionTaskService` |
