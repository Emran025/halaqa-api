...
class AcceptSessionController extends Controller
{
    public function __invoke(LiveSession $session, SessionTransitionService $service): JsonResponse
    {
        Gate::authorize('accept', $session);

        return SessionResponseResource::make($service->accept($session))->response()->setStatusCode(200);
    }
}
