...
تنتقل تغييرات الصفحة، الآية المحددة، موضع المؤشر، وطلب إعادة القراءة عبر DataChannel P2P لتظهر فورًا للطرف الآخر. هذه أحداث عرض مؤقتة لا تُعامل كمصدر حقيقة. وعندما يريد العميل تثبيت الإصدار والصفحة والآية أو نطاق التلاوة رسميًا، يرسلها إلى `PUT /sessions/{sessionId}/mushaf-state` ليحفظها `SaveSessionMushafStateService` في `session_mushaf_states`.
